						...
						<div id="hidden" class="page-header">
							...
							<div class="page-rightheader">
								<div class="btn btn-list">
									...
									<a href="{{route('bancarisations.create')}}" id="hidden"  style="background:#262626; color:#fff" class="btn btn"><i class="fe fe-plus mr-1"></i> Nouvelle demande </a>
									@endcan
															...
															<table class="table table-bordered border-top text-nowrap" id="example1">
																<thead>
																	<tr>
																		...
																		<th class="border-bottom-0 w-20">Numéro-compte/Nom société</th>
																...
																<tbody>
																	@if(!empty($bancarisations) && $bancarisations->count())
																	@foreach($bancarisations as $bancarisation)
																	 <tr>
																	 <th scope="row">{{$bancarisation->id}}</th>
																		<td>{{\Carbon\Carbon::parse($bancarisation->created_at)->format('d/m/Y')}}</td>


                                                                        @if ( $bancarisation->renseigne_id == 2)
                                                                                @if ($bancarisation->entreprise_id !== Null)
                                                                                <td>{{$bancarisation->entreprise->code}} - {{$bancarisation->entreprise->name}}</td>
                                                                                @elseif($bancarisation->society_id !== Null)
                                                                                    <td>{{$bancarisation->society->code}} - {{$bancarisation->society->name}}</td>
                                                                                @else

                                                                                @endif

                                                                                @if ($bancarisation->entreprise_id !== Null)
                                                                                    <td>{{$bancarisation->entreprise->tel}}</td>
                                                                                @elseif($bancarisation->society_id !== Null)
                                                                                    <td>{{$bancarisation->society->tel}}</td>
                                                                                @else

                                                                                @endif
                                                                        @else
                                                                            <td>{{$bancarisation->name}}</td>
                                                                            <td>{{$bancarisation->tel_com}}</td>
                                                                        ...
                                                                        @endif
																		 ...
																		 <td>
																			...
																			@can('manage-bancaires')
																			    <a href="{{ route('bancarisations.print' , ['bancarisation' => $bancarisation->id]) }}" style="background-color:#eee;" class="btn btn-">🖨️</a>
																		        {{-- @can('edit-customers')
