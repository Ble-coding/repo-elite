						...
						<div id="hidden" class="page-header">
							...
							<div class="page-rightheader">
								<div class="btn btn-list">
									<a href="{{route('visiteurs.create')}}" id="hidden"  style="background:#262626; color:#fff" class="btn btn"><i class="fe fe-plus mr-1"></i> Nouveau particulier </a>
									...
									{{-- <a class="btn btn-success" data-target="#modaldemo1" data-toggle="modal" href="">Nouveau visiteur</a> --}}
										</div>
							</div>
						</div>
                        <!--End Page header-->
											...
											<div class="e-table">
												<div class="table-responsive table-lg mt-3">
													<table class="table table-bordered border-top text-nowrap" id="example1">
														...
														<tbody>
															@if(!empty($visiteurs) && $visiteurs->count())
															@foreach($visiteurs as $visiteur)
															 <tr>
															 <th scope="row">{{$visiteur->id}}</th>
															 @php
																$image = DB::table('visiteurs')->where('id', 1)->first();
															   ...
															   @endphp  
																...
																<td>{{$visiteur->code}}</td>
																 <td>{{$visiteur->name}} {{$visiteur->prename}}</td>       
																 <td>{{\Carbon\Carbon::parse($visiteur->datenaiss)->format('d/m/Y')}}</td> 
																 <td>{{$visiteur->email}}</td> 
																 <td>{{$visiteur->tel}}</td>   
																 <td>{{implode(' , ', $visiteur->user->roles()->pluck('name')->toArray())}}</td>                                           
																 <td>
																	{{-- @can('show-visiteurs') --}}
																	<a href="{{ route('visiteurs.show' , ['visiteur' => $visiteur->id]) }}" style="background-color:#fff" class="btn btn-">👀</a>
																	{{-- @endcan --}} 
																	@can('edit-visiteurs')
																	   <a href="{{ route('visiteurs.edit' , ['visiteur' => $visiteur->id]) }}" style="background-color:#262626;" class="btn btn-">✏️</a>
																	   @endcan
																	   @can('delete-visiteurs')
																	   <form class="d-inline" method="POST" action="{{ route('visiteurs.destroy' , ['visiteur' => $visiteur->id] ) }}">
																		@csrf
																		@method('DELETE')
																		<button  onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce particulier ?');" type="submit" style="background:#ff0017;" class="btn btn">🗑️</a>
																</form>   
																	   @endcan
											...
											<div class="e-table">
												<div class="table-responsive table-lg mt-3">
													<table class="table table-bordered border-top text-nowrap" id="example2">
														...
														<tbody>
															@if(!empty($supps) && $supps->count())
															@foreach($supps as $par)
															<tr>
															...
															@endphp  
																...
																{{-- <td>{{$par->email}}</td>  --}}
																{{-- <td>{{$par->tel}}</td>      --}}
																<td>{{implode(' , ', $par->user->roles()->pluck('name')->toArray())}}</td>                                                  
																<td>
																			
																	@can('delete-visiteurs')
																	
																<form method="POST"  class="d-inline" 
																action="{{route('visiteurs.restore' , ['visiteur' => $visiteur->id]) }}">
																	@csrf
																	@method('PUT')																			
																	<button style="background:#eee; color:#262626" class="btn btn" onclick="return confirm('Êtes-vous sûr de vouloir restaurer les infos du visiteur ?');" type="submit">✔️</button>																			
																	</form>
