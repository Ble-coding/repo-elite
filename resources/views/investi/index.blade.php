															 ...
															 <td>{{substr($v, 12, 35)}}</td> --}}
																...
																{{-- {{ $investi}} --}}
																	...
																	<td>{{\Carbon\Carbon::parse($investi->created_at)->format('d/m/Y')}}</td>
																		 ...
																		 @if ($investi->forfait_id == 4 ||  $investi->forfait_id == 5)
																			 <div style="display:none">{{$dateviw = \Carbon\Carbon::parse($investi->created_at)->addMonth(12)->subDay(1)->format('d/m/Y')}}</div>
																		 @else
																		...
																		@else
																			@if ($investi->forfait_id == 1)
																				<div style="display:none">{{$dateviw = \Carbon\Carbon::parse($investi->created_at)->addMonth(6)->subDay(1)->format('d/m/Y')}}</div>
																				<td>{{$dateviw}} </td>
																	...
																	@if ($investi->jalon == 'Oui')
																				@if ($investi->forfait_id == 4)
																						...
																						<div style="display:none" >{{$vio = $viol}} </div>
																	...
																	@if ($investi->forfait_id == 4)
																		<div style="display:none" >{{$v0 = 200000}}</div>
																			<td>{{ number_format($v0, 0, ',', ' ') }}</td>
																	@elseif($investi->forfait_id == 5)
																		<div style="display:none" >{{$v0 = 400000}}</div>
																		<td>{{ number_format($v0, 0, ',', ' ') }}</td>
																	@else
																		...
																		<div style="display:none" >{{$v1 = $v0/$investi->forfait->duree}}</div>  --}}
																					@if ($investi->forfait_id == 1)
																						<div style="display:none" >{{$v0 = ($investi->montant * (($investi->forfait->pourcentageM  * 1) /100))}}</div>
																					@elseif($investi->forfait_id == 2)
																							<div style="display:none" >{{$v0 = ($investi->montant * (($investi->forfait->pourcentageM  * 1) /100))}}</div>
																					@else
																							<div style="display:none" >{{$v0 = ($investi->montant * (($investi->forfait->pourcentageM  * 3) /100))}}</div>

																					@endif

																					@if ($investi->forfait_id == 1)
																						<div style="display:none" >{{$v1 = $v0}}</div>
																					@elseif($investi->forfait_id == 2)
																						<div style="display:none" >{{$v1 = $v0}}</div>
																					@else
																					<div style="display:none" >{{$v1 = $v0/3}}</div>
																					@endif
																	...
																	@endif
																		...
																		{{-- @if ($investi->forfait_id == 4)
																		<div style="display:none" >{{$vi = $v0 * $investi->forfait->duree}} </div>
																		...
																		@endif --}}
																					...
																					@else
																						@if ($investi->forfait_id == 1)
																							<div style="display:none" >{{$v0 = $vtrack = ($v1) + $investi->montant }}</div>
																							<td>{{ number_format($vtrack, 0, ',', ' ') }}</td>
																						@elseif($investi->forfait_id == 2)
																							<div style="display:none" >{{$vtrack = ($v1 * 2) + $investi->montant }} </div>
																							<td>{{ number_format($vtrack, 0, ',', ' ') }}</td>
																						@else
																							<div style="display:none" >{{$vtrack = $v1 +  $v0 + $investi->montant }} </div>
																							<td>{{ number_format($vtrack, 0, ',', ' ') }}</td>
																						@endif
...
{{--
																			...
																			@endif --}}
																				...
																				@if ($investi->jalon == 'Oui')
																						@if($investi->forfait_id == 1 ||
																						$investi->forfait_id == 2 || 
																						$investi->forfait_id == 3  )
																						...
																						@elseif($investi->forfait_id == 4 ||
																						$investi->forfait_id == 5  )

																							@if ($investi->particulier_id !== Null)
																								@if ($investi->particulier->status == 0)
																									@if ( \Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($investi->created_at))  &&  \Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($investi->created_at)->addDay(360)) )
																										<a href="{{ route('investir.diminishes.stored' , ['investi' => $investi->id]) }}" style="background-color:#eee;" class="btn btn-"><i class="fe fe-minus mr-1"></i></a>
																									@else
																...
																<tr>
																  ...
																  {{-- <td>{{$transaction->suppleant->name}} {{$transaction->customer->prename}}</td> --}}
																	...
																	@if ($transaction->forfait_id == 4)
																					...
																					{{-- <td> {{$v1}} </td> 	   																						   --}}
																					<div style="display: none">{{$vi = $v1}} </div>
																					<td>{{ number_format($vi, 0, ',', ' ') }}</td>
																				@elseif ($transaction->forfait_id == 5)
																					<div style="display:none" >{{$v0 = ($transaction->montant * (($transaction->forfait->pourcentageM) /100 ) * $transaction->forfait->duree) }}</div>
																		...
																		@else
																				...
																				@if ($transaction->forfait_id == 4)
																					...
																					{{-- <td> {{$v1}} </td> 	   																						   --}}
																					<div style="display: none">{{$vi = $v1 * $transaction->forfait->duree}} </div>
																					<td>{{ number_format($vi, 0, ',', ' ') }}</td>
																				@elseif ($transaction->forfait_id == 5)
																					<div style="display:none" >{{$v0 = ($transaction->montant * (($transaction->forfait->pourcentageM) /100 ) * $transaction->forfait->duree) }}</div>
																				...
																				@else
																						...
																						@if ($transaction->forfait_id == 1)
																							<div style="display:none" >{{$v0 = ($transaction->montant * (($transaction->forfait->pourcentageM) /100 ) * 1) }}</div>
																							{{-- <div style="display:none" >{{$v1 = $v0/$transaction->forfait->duree}}</div>  --}}
																							<div style="display:none" >{{$v1 = $v0}}</div>
																							{{-- <td> {{$v1}} </td> 	   																						   --}}
																							<div style="display: none">{{$vi = ($v1) + $transaction->montant}} </div>
																							<td>{{ number_format($vi, 0, ',', ' ') }}</td>
																						@elseif ($transaction->forfait_id == 2)
																							<div style="display:none" >{{$v0 = ($transaction->montant * (($transaction->forfait->pourcentageM) /100 ) * 1) }}</div>
																							{{-- <div style="display:none" >{{$v1 = $v0/$transaction->forfait->duree}}</div>  --}}
																						...
																						@else
																							...
																							{{-- <div style="display:none" >{{$v0 = ($transaction->montant * (($transaction->forfait->pourcentageM) /100 ) * $transaction->forfait->duree) }}</div>  --}}
																							<div style="display:none" >{{$v0 = ($transaction->montant * (($transaction->forfait->pourcentageM) /100 ) * 3) }}</div>
																							{{-- <div style="display:none" >{{$v1 = $v0/$transaction->forfait->duree}}</div>  --}}
																							<div style="display:none" >{{$v1 = $v0/3}}</div>
																							{{-- <td> {{$v1}} </td> 	   																						   --}}
																							<div style="display: none">{{$vi = $v0 + $v1 + $transaction->montant}} </div>
																							<td>{{ number_format($vi, 0, ',', ' ') }}</td>
																						@endif
																	...
																	@can('manage-users')
																			...
																			@if ($transaction->jalon == 'Oui')
																					@if ($transaction->forfait_id == 4 || $transaction->forfait_id == 5)
																						<div style="display: none">{{$vi = 12}} </div>
																						<td>{{$vi}}</td>
																					@else
...
<script type="text/javascript">
 ...
 document.getElementById("ccform").style.visibility='visible';
	 ...
	 function mySelectchoice(){
		getValue = document.getElementById("choice_id").value;
		if(getValue == 1){
