																		...
																		<tbody>
																			...
																			@foreach($deposits as $deposit)
																			 ...
																			 <td>{{$deposit->entreprise->code}}</td>
																				 <td>{{$deposit->entreprise->name}}</td> 
																			 <td>{{$deposit->entreprise->name_gerant}} {{$deposit->entreprise->prename_gerant}} {{$deposit->entreprise->email}} {{$deposit->entreprise->tel}}</td>
																				 	 {{--<td>{{$deposit->email}}</td>            --}}
																				 <td>{{ number_format($deposit->montant, 0, ',', ' ') }}</td>        
																				 {{-- <td>{{ number_format($deposit->total_quantity, 0, ',', ' ') }}</td>                                            --}}
																				 <td> 
																					{{-- @can('show-deposits') --}}
																					{{-- <a href="{{ route('deposit.deposits.show' , ['deposit' => $deposit->id]) }}" style="background-color:#fff" class="btn btn-">👀</a> --}}
																					{{-- @endcan --}}
														...
														<tbody>
															...
															@foreach($withadrals as $withadral)
															 ...
															 <td>{{$withadral->sold->entreprise->code}}</td> 
																 <td>{{$withadral->sold->entreprise->name}}</td>     
																 <td>{{$withadral->sold->entreprise->name_gerant}} {{$withadral->sold->entreprise->prename_gerant}} {{$withadral->sold->entreprise->email}} {{$withadral->sold->entreprise->tel}}</td>                                                  
																 <td>{{ number_format($withadral->montant, 0, ',', ' ') }}</td>      
																 {{-- <td>{{ number_format($withadral->total_quantity, 0, ',', ' ') }}</td>                                          --}}
																 <td>
																	@if ( \Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($withadral->created_at))  &&  \Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($withadral->created_at)->addDay(3))    )

																	@else
																
																	
																		@can('edit-withadrals')
																		<a href="{{ route('withadral.withadrals.edit' , ['withadral' => $withadral->id]) }}" style="background-color:#262626;" class="btn btn-">✏️</a>
																		@endcan
																	@endif

																	@can('delete-withadrals')
																	<form class="d-inline" method="POST" action="{{ route('withadral.withadrals.destroy' , ['withadral' => $withadral->id]) }}">
																	 @csrf
																	 @method('DELETE')
												...
												<table class="table table-bordered border-top text-nowrap" id="example-delete">
													...
													<tbody>
														...
														<td>{{$ent->deleted_at}}</td>			
															<td>{{$ent->entreprise->code}}</td>
															<td>{{$ent->entreprise->name}}</td>      
															<td>{{$ent->entreprise->name_gerant}} {{$ent->entreprise->prename_gerant}} {{$ent->entreprise->email}}  {{$ent->entreprise->tel}}</td>  
															<td>{{$ent->montant}}</td>
															...
															<td>
																...
																<form method="POST"  class="d-inline" action="{{route('deposit.deposits.restore' , ['deposit' => $deposit->id]) }}">
																	@csrf
																	@method('PUT')																			
										...
										<div class="table-responsive table-lg mt-3">
											<table class="table table-bordered border-top text-nowrap" id="example-1">
												...
												<tbody>
													...
													<th scope="row">{{$ent->id}}</th>
													<td>{{$ent->deleted_at}}</td>			
														<td>{{$ent->sold->entreprise->code}}</td>
														<td>{{$ent->sold->entreprise->name}}</td>
														<td>{{$ent->sold->entreprise->name_gerant}} {{$ent->sold->entreprise->prename_gerant}} {{$ent->sold->entreprise->email}}  {{$ent->sold->entreprise->tel}}</td>       
														<td>{{$ent->montant}}</td> 
														<td>{{implode(' , ', $ent->user->roles()->pluck('name')->toArray())}}</td>                                                  
														<td>
															...
															<form method="POST"  class="d-inline" action="{{route('withadral.withadrals.restore' , ['withadral' => $withadral->id]) }}">
																@csrf
																@method('PUT')																			
