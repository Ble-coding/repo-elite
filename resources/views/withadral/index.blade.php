						...
						<div id="hidden" class="page-header">
							...
							<div class="page-rightheader">
								<div class="btn btn-list">
									<a href="{{route('withadral.withadrals.create')}}" id="hidden"  style="background:#262626; color:#fff" class="btn btn"><i class="fe fe-plus mr-1"></i> Nouveau retrait </a>
														...
														<table class="table table-bordered border-top text-nowrap" id="example1">
															...
															<tbody>
                                                                ...
                                                                @foreach($withadrals as $withadral)
																 ...
																 <td>{{\Carbon\Carbon::parse($withadral->created_at)->format('d/m/Y')}}</td> 
                                                                     <td>{{$withadral->entreprise->name}}</td>
																	 <td>{{$withadral->entreprise->name_gerant}} {{$withadral->entreprise->prename_gerant}}</td>    
																	 <td>{{$withadral->entreprise->tel}}</td>            
																	 <td>{{ number_format($withadral->montant, 0, ',', ' ') }}</</td>                                            
                                                                     <td>
																		{{-- @can('show-withadrals') --}}
																		{{-- <a href="{{ route('withadral.withadrals.show' , ['withadral' => $withadral->id]) }}" style="background-color:#fff" class="btn btn-">👀</a> --}}
																		{{-- @endcan --}}
																		@can('edit-withadrals')
