...
@section('page-header')
						...
						<div id="hidden" class="page-header">
							<div class="page-leftheader">
								<h4 class="page-title mb-0">Liste des retraits des clients</h4>
								<ol class="breadcrumb">
							...
							<div class="page-rightheader">
								<div class="btn btn-list">
									<a href="{{route('diminish.diminishes.create')}}" id="hidden"  style="background:#262626; color:#fff" class="btn btn"><i class="fe fe-plus mr-1"></i> Nouveau retrait </a>
															...
															<tbody>
                                                                ...
                                                                @foreach($diminishes as $diminish)
																 ...
																 <td>{{\Carbon\Carbon::parse($diminish->created_at)->format('d/m/Y')}}</td> 
                                                                     <td>{{$diminish->customer->name}} {{$diminish->customer->prename}}</td>
																	 <td>{{$diminish->customer->tel}}</td>                                            
																	 <td>{{$diminish->customer->email}}</td>                                                        
																	 <td>{{ number_format($diminish->montant, 0, ',', ' ') }}</td>                                            
                                                                     <td>
																		{{-- @can('show-diminishs') --}}
																		{{-- <a href="{{ route('diminish.diminishs.show' , ['diminish' => $diminish->id]) }}" style="background-color:#fff" class="btn btn-">👀</a> --}}
																		{{-- @endcan --}}
																		@can('edit-diminishes')
                                                                           <a href="{{ route('diminish.diminishes.edit' , ['diminish' => $diminish->id]) }}" style="background-color:#262626;" class="btn btn-">✏️</a>
																		   @endcan
																		   @can('delete-diminishes')
